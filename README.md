...
(<https://mixcr.readthedocs.io/en/master/index.html>). Sample data in
the object VDJDBCMVPairedInput is obtained from
<https://vdjdb.cdr3.net/>.

A CDR3 sequence can be broken down into k-mers. SPAN-TCR extracts the
k-mers contributing to the normalized position of amino acids within
CDR3 sequences

Basic workflow: 1. Prepare data. a. Create an object AminoAcidFilter
which contains relevant amino acid information b. Organize TCR data with
columns CDR3\|gene\|Vgene\|Jgene\|score\|id i. CDR3 - amino acid
sequence (from MixCR) ii. gene - TRA or TRB iii. Vgene/Jgene - TRAV1-2,
TRBJ2-2, etc. iv. score - a score describing the quality of the TCR v.
id - an integer used to track paired CDR3s c. Define a scoring function
i. The score column is often taken from an external data source ii.
Define a scoring function to scale the score appropriately iii. This is
up to your discretion d. Define a weighting function i. A k-mer has its
maximum binding character likely at the center of its position ii. The
weighting function determines how the influence of a k-mer increases or
decreases from its center

2.  Generate CDR3Breakdown objects using SPANTCR

<!-- -->

1.  The majority of k-mers are rare and likely have minimal impact on
    binding
2.  Set a significance cutoff to determine which k-mers to visualize
3.  This example uses 5^x as the score function, a linear weighting as
    the weight function, and has a cutoff of 0.03
4.  k-mers of length 2 are analyzed and 100 bins are used
5.  k-mers are ordered by their hydrophobicity
...
SPANTCR plots can be generated using ggplot2 Ordering output by the same
AAOperation will stack bar plots appropriately The levels of factor
(Window) will match the AAOperation order
...
ComparisonFunction Here we compare Levenshtein distance between VDJDBCMV
Paired CDR3s and their nearest neighbor in the set
...
or in specific windows using SearchIterator EntropyScan is a wrapper for
SearchIterator Entropy across the entire range of k-mers and positions
is summarized in the first list element of the output
